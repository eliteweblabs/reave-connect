...
// ---------------------------------------------------------------------------
// 4. Settings page — store API key in DB as fallback (wp-config.php preferred)
// ---------------------------------------------------------------------------
...
function reave_connect_settings_page() {
    ...
    <div class="wrap">
        <h1>Reave Connect</h1>
        <p>This plugin allows <a href="https://reave.app/" target="_blank">Reave Automation</a> to manage this WordPress site remotely — posts, pages, media, plugins, head scripts, cache, and options.</p>
        <form method="post">
...
function reave_connect_health(): array {
    ...
    global $wpdb;
    $theme = wp_get_theme();
    $head_scripts = get_option( 'reave_head_scripts', [] );
    return [
        'ok'             => true,
        'site_url'       => get_site_url(),
        'site_name'      => get_bloginfo( 'name' ),
        ...
        'plugin_version' => REAVE_CONNECT_VERSION,
        'head_scripts'   => array_column( is_array( $head_scripts ) ? $head_scripts : [], 'handle' ),
    ];
}
